...
class AclShell extends Shell
{
    ...
    public function startup(): void
    {
        ...
        if (
            ...
            $out .= __d(
                'cake_acl',
                'Error: Your current CakePHP configuration is set to an ACL implementation other than DB.'
            ) . "\n";
            $out .= __d(
                'cake_acl',
                'Please change your core config to reflect your decision ' .
                'to use DbAcl before attempting to use this script'
            ) . "\n";
            $out .= "--------------------------------------------------\n";
    ...
    public function create()
    {
        ...
        if (is_string($data) && $data !== '/') {
            ...
            $this->abort(
                __d('cake_acl', '/ can not be used as an alias!') .
                __d('cake_acl', '  / is the root, please supply a sub alias')
            );
        }
    ...
    public function delete()
    {
        ...
        if (!$this->Acl->{$class}->delete($entity)) {
            $this->abort(
                __d('cake_acl', 'Node Not Deleted') .
                __d('cake_acl', 'There was an error deleting the {0}. Check that the node exists.', [$class]) .
                "\n"
            );
        }
    ...
    public function setParent()
    {
        ...
        if (!$this->Acl->{$class}->save($entity)) {
            $this->out(
                __d(
                    'cake_acl',
                    'Error in setting new parent. Please make sure the parent node exists, ' .
                    'and is not a descendant of the node specified.'
                )
            );
        } else {
    ...
    public function getOptionParser(): ConsoleOptionParser
    {
        ...
        $parser->setDescription(
            ...
            'parser' => [
                ...
                'epilog' => __d(
                    'cake_acl',
                    'You can use `root` as the parent when creating nodes to create top level nodes.'
                ),
                'arguments' => [
                    'type' => $type,
                    'parent' => [
            ...
            'parser' => [
                'description' => [
                    ...
                    __d(
                        'cake_acl',
                        'This command is useful in determining the inheritance of permissions ' .
                        'for a certain object in the tree.'
                    ),
                ],
                'arguments' => [
                    'type' => $type,
            ...
            'parser' => [
                'description' => [
                    __d(
                        ...
                        '(and its children, if any) will have ALLOW access to the specified ACO action ' .
